...
function filter(data, route) {
    ...
    if (typeof schema === 'string') {
        validator = req.validatorManager.get(schema);
    } else {
        validator = new jsonInspector.Validator(schema, {
            required: false,
            filterData: true,
            failOnUnexpectedData: false,
            failOnFirstErr: false,
            nullable: true,
        }, req.validatorManager);
    }

    validator.validate(data, null, {
        //if we get validator from global validatorManager, validationError options
        //can be overwritten thus we need to ensure correct errors will be throwed
        validationError: jsonInspector.ValidationError,
        validationMultiError: jsonInspector.ValidationMultiError
    });
