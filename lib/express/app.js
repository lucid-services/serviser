...
/**
 * when called with specific BIServiceSDK object, that sdk is connected to the app.
 ...
 * @param {BIServiceSDK} [sdk] - mutually exclusive with the `options` argument, when SDK instance is provided, it's connected to the app
 * @param {Object}       [options] - mutually exclusive with the `sdk` argument, options are provided to SDK constructor when instantiating a new instance
 *
...
App.prototype.useSDK = function(key, sdk, options) {
    ...
    if (sdk === undefined) {
        ...
        if (!_.isPlainObject(sdk) || !sdk.npm) {
            throw new Error(`"services:${key}:npm" config options not found. Cant connect sdk.`);
        }

        var sdkVersions = require(sdk.npm);
