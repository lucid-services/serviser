 ...
 * @constructor
 **/
function App(appManager, config, models) {
...
App.prototype.$init = function(options) {
    ...
    this.on('status', function(status) {
        if (   status === AppStatus.ERROR
            ...
            console.error('This node will shutt down to prevent any further data damage');
            process.exit(1);
        }
    });
 ...
 * @return {SessionStore}
 */
App.prototype.connectSession = function(store) {
...
App.prototype.clone = function() {
    var app         = new App(this.appManager, this.config, this.models);
    ...
    process.nextTick(function() {
        app.$init(originalApp.initOptions);
    });
