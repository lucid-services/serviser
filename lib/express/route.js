...
/**
 * returns relative route endpoint to the hostname
 *
...
Route.prototype.getUrl = function getUrl(pathParams, queryParams) {
    var url = this.Router.getUrl() + this.options.url;
...
Route.prototype.restrictByClient = function(options) {
    ...
    app.once('post-init', function() {
        if (   !this.sdk
            || !this.sdk.privateDepot
            || !this.sdk.privateDepot['v1.0']
            || !options.depot.serviceName
        ) {
            this.emit('error', new RouteError('Misconfigured `bi-depot` service dependency'));
        }
    });
