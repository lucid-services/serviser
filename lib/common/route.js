 ...
 * NOTE: only `json`/`urlencoded`/`raw`/`text` media types have fallback parsers defined. if you want to support any other data format, you must also provide parsing function with it...
 *
 ...
 * With `res.filter({some: 'data'}).json()` conjuction, data can be filtered
