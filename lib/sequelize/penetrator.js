...
let cache        = require('../sequelize/cache/cache');

let sequelizePenetrated = false;
...
module.exports.instanceMethods = {
    addToJson            : addToJson,
    refreshCache         : cache.refreshInstanceCache,
    destroyCache         : cache.destroyInstanceCache,
    normalize            : normalize,
...
module.exports.classMethods = {
    ...
    buildFromCache           : cache.buildFromCache(sequelize) // Temporary disabled cache penetration
}
...
function createAssociationList() {
    ...
    for (var index in this.associations) {
        ...
        var name = ass.target.name.toCamelCase().ucfirst();
...
function getAssociationArray(options) {
    ...
    var Models   = require(libs + 'models/group');
    var included = [];
    var self     = this;
    ...
    function loadAssociations(model, includeNestedAssociations, opt) {
        ...
        var parentName = options.name.singular.toCamelCase().ucfirst();
...
function search(input, options) {
    ...
    if (refreshCache && (!this.options.cache || !this.options.cache.storage)) {
        return Promise.reject(new RequestError('Cache operations not supported.'));
    }
        ...
        for (var i in relations) {
            if (relations.hasOwnProperty(i)) {
                ...
                if (input.hasOwnProperty(name)) {
                    return Promise.reject(new RequestError('Relation filtering not supported if count_only flag is supplied.'));
                }
            }
        }
    }
    ...
    filter.include.forEach(function(assoc) {
        var assocName = assoc.model.options.name.singular.toCamelCase().ucfirst();
    ...
    for (var name in input) {
        ...
        if (relations.hasOwnProperty(name)) {
            ...
            var model = Models[relation.model];
            var condition = {};
    ...
    if  (Array.isArray(filter.order) ) {
        for (var i in filter.order) {
            ...
            if (Array.isArray(order) && order.length === 3) {
                var relName = order[0].toCamelCase().ucfirst();
                if ( !this.associationList.hasOwnProperty(relName) ) {
                    return Promise.reject(new RequestError('The relation was not found. Can not order by it'));
                }
