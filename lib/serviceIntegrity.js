...
var Inspector = {
    ...
    inspectPostgres: Promise.method(function inspectPostgres(app) {
        ...
        var dialect = app.sequelize.options.dialect;
        var q;
        ...
        return app.sequelize.query(q, {
            type: app.sequelize.QueryTypes.SELECT
        }).then(function (results) {
            var version = results[0]['server_version'];
            var expectedVersion = app.config.get(`${dialect}:version`);

            return self.inspectVersion(version, expectedVersion, dialect);
        });
    }),
