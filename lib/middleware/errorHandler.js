...
function errorHandler(err, req, res, next) {
    ...
    if (err === undefined) {
        ...
        //TODO add support for async promises and define json response fallback
        //when we emit the `error-reponse` event
        res.status(err.code);
        if (app.listenerCount('error-response')) {
            return app.emit('error-response', err, res);
        }
        res.json(err);
