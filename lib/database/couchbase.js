...
CouchbaseCluster.prototype.openBucketSync = function(name) {
    ...
    if (!this.options.buckets.hasOwnProperty(name)) {
        throw new ServiceError('There is no config record for the bucket: ' + name);
    }
...
CouchbaseCluster.prototype.openBucket = function(name) {
    ...
    if (!this.options.buckets.hasOwnProperty(name)) {
        return Promise.reject(new ServiceError('There is no config record for the bucket: ' + name));
    }
