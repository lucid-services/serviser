...
var assertions = module.exports.assertions = {
    ...
    $like: function() {
        ...
        function like() {
            if (typeof this.filter !== 'string' || typeof this.val !== 'string') {
                return false;
            }
    ...
    $iLike: function() {
        ...
        if (_.isPlainObject(this.filter)) {
            for (var i = 0, len = (this.filter.$any || []).length; i < len; i++) {
                var val = this.filter.$any[i];
                if (typeof val === 'string') {
                    this.filter.$any[i] = val.toLowerCase();
                }
            }
        } else {
 ...
 * @param {object} where - filter definition object
 * @param {object} [options]
 * @param {string} [options.dataProperty='dataValues']
 * @return {array}
 */
function filterData(data, where, options) {
    var defaults = {
        dataProperty: 'dataValues'
    };
    ...
    return data.filter(function(obj) {
        if (options.dataProperty) {
            obj = _.get(obj, options.dataProperty);
        }
        return buildWhere(obj, where || {}, '$and' )();
    });
}
