...
describe('functional server', function() {
    before(function() {
        ...
        return Promise.fromCallback(function(cb) {
            return findPorts('127.0.0.1', 8000, 8025, function(ports) {
                ...
                if (!ports.length || ports.length < 2) {
                    return cb(new Error('Could not allocate free port in range: 8000-8025'));
                }
                cb(null, ports);
            });
        }).bind(this).then(function(ports) {
            service.config.set('apps:app1:listen', ports.pop());
            service.config.set('apps:app2:listen', ports.pop());
            return service.listen();
        }).then(function() {
            this.server = service.appManager.get('app1').expressApp;
            this.server2 = service.appManager.get('app2').expressApp;
        });
    });
    ...
    describe('app1 - ajv allErrors=false', function() {
        [
            {
    ...
    describe('app2 - ajv allErrors=true', function() {
        [
            {
                should: 'return correctly formated validation error',
                url: '/validate/body',
                ...
                response: {
                    ...
                    failures: [
                        {dataPath: '.username', message: 'should be string'},
                        {dataPath: '.email', message: 'should match format "email"'},
                    ]
                }
            },
        ].forEach(function(req, index) {
            it(`should ${req.should}`, function(done) {
                ...
                let test = request(this.server2)[req.method](req.url + query);
                if (req.contentType) {
                    test.set('Content-Type', req.contentType);
                }
